...
| `api.yaml` | `monitor.api.load_api_config` | Read-only FastAPI bind host/port, SQLite path, collector-stale / gap windows, poll interval hint, CORS — WHI-757; **`pnl_cache_ttl_s`** process-local PnL v2 snapshot TTL (default 2s, align with poll) — WHI-766. |
