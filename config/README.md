...
| `metrics.yaml` | `monitor.metrics.load_metrics_config` | Size ladder, session hours, PnL v2 search knobs, **`max_abs_amm_spread_bps`** (WHI-822/WHI-964 pricing-anomaly guard; default 300, bot-aligned; `null` disables). Venue fee/gas **overridden** at assembly from the market file `costs:`. |
...
| `api.yaml` | `monitor.api.load_api_config` | FastAPI bind (`127.0.0.1`), default `market`, sqlite path, **`static_dir`** (optional Next export path; mount only if dir exists — WHI-979 / ADR-0003; default `/opt/xstocks/www` for VPS rsync target), `collector_stale_ms` (process liveness) + `quote_max_age_ms` (PnL age annotate, WHI-821), PnL cache TTL, CORS (local dogfood only). |
