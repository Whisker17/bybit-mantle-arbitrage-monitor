...
| Asset withdrawal fee | per-pair `asset_withdrawal_fee_tokens` (inventory) | Dir2: `tokens × listed mid` (`dm_mid × bybit.multiplier`). Measured on all seven liquid xStocks (WHI-1090). Unmeasured → `withdrawal_fee_kind=unknown` (never silent 0); unknown dir2 is excluded from overview Net / Cap $/d. **Market-agnostic:** binance-pancake has no measured asset schedule (`BStocksPair` → `asset_fee_tokens=None`), so its dir2 is excluded the same way. Liquid Bybit AMM pairs without a fee fail config load. Capture now charges the pair fee on measured dir2 (Cap $/d was silently unpriced for HOODx/CRCLx/NVDAx too). |
