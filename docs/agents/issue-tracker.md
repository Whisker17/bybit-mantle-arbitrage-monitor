...
Issues and PRDs for this repo live in **Linear**, project **"Mantle <> Bybit Arbitrage Monitor"**, team
`Whisker-Personal`.
...
- **Create / update an issue**: `linear.save_issue({...})`. When creating, `title` and
  `team` are required; also set `project` to `"Mantle <> Bybit Arbitrage Monitor"` so it's scoped
...
to `"Mantle <> Bybit Arbitrage Monitor"`). Follow the canonical structure in
...
2. **Blocking edges**: wire them as Linear's native **`blocked-by` / `blocks`
   ...
   mirror.
3. **Scoping**: every issue gets `project: "Mantle <> Bybit Arbitrage Monitor"` and, if the set belongs
...
- **Map**: one Linear issue labelled `wayfinder:map`, holding the Notes /
  ...
  project: "Mantle <> Bybit Arbitrage Monitor", labels: ["wayfinder:map"] })`. Create the label first
...
- **Child ticket**: an issue whose **`parent`** is the map — Linear's native sub-issue
  ...
  `linear.save_issue({ title, team, project: "Mantle <> Bybit Arbitrage Monitor", parent: <map-id>,
